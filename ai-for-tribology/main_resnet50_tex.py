...
from datasets.data_tribology import TrainTransform
...
if __name__ == '__main__':
    ...
    lr_fig_path = f'./lr-finder-ResNet50_tex.pdf'
    ...
    for epoch in range(EPOCHS):
        ...
        if valid_loss < best_valid_loss:
            ...
            # Saving the model
            save_path = f'./model-ResNet50_tex.pth'
    ...
    cm_fig_path = f'./confusion-matrix-ResNet50_tex.pdf'
    ...
    pca_path = f'./output-pca-ResNet50_tex.pdf'
    ...
    filtered_image_path = f'./output-filtered-image-ResNet50_tex.pdf'
    plot_filtered_images(filtered_image_path, images, filters, N_FILTERS)

    filters_path = f'./output-filters-ResNet50_tex.pdf'
    plot_filters(filters_path, filters)

    tsne_fig_path = f'./output-tsne-ResNet50_tex.pdf'
    plot_dataloader_tsne(tsne_fig_path, test_iterator)

    # Grad-CAM
    grad_cam_path = f'./output-gradcam-ResNet50_tex.pdf'

    target_layer = model.layer4[-1]
    visualizations = []

    images = images[:5]
    true_labels_list = labels[:5].tolist()
    predicted_labels_list = pred_labels[:5].tolist()

    for img in images:
        visualization = get_grad_cam(model, target_layer, img.unsqueeze(0).to(device))
        visualizations.append(visualization)
    
    plot_grad_cam(images, visualizations, predicted_labels_list, true_labels_list, classes, grad_cam_path)
